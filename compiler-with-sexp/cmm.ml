...
type raise_kind = Compiler_without_sexp.Cmm.raise_kind =
  | Raise_withtrace
  | Raise_notrace
[@@deriving sexp_of]
...
and expression = Compiler_without_sexp.Cmm.expression =
  ...
  | Ccatch of
      rec_flag
      * (int * (Backend_var.With_provenance.t * machtype) list * expression) list
      * expression
...
include (
  Compiler_without_sexp.Cmm :
    module type of struct
      include Compiler_without_sexp.Cmm
    end
    with type codegen_option := Compiler_without_sexp.Cmm.codegen_option
     and type data_item := Compiler_without_sexp.Cmm.data_item
     and type expression := Compiler_without_sexp.Cmm.expression
     and type float_comparison := Compiler_without_sexp.Cmm.float_comparison
     and type fundecl := Compiler_without_sexp.Cmm.fundecl
     and type integer_comparison := Compiler_without_sexp.Cmm.integer_comparison
     and type label := Compiler_without_sexp.Cmm.label
     and type machtype := Compiler_without_sexp.Cmm.machtype
     and type machtype_component := Compiler_without_sexp.Cmm.machtype_component
     and type memory_chunk := Compiler_without_sexp.Cmm.memory_chunk
     and type operation := Compiler_without_sexp.Cmm.operation
     and type phantom_defining_expr := Compiler_without_sexp.Cmm.phantom_defining_expr
     and type phrase := Compiler_without_sexp.Cmm.phrase
     and type raise_kind := Compiler_without_sexp.Cmm.raise_kind
     and type rec_flag := Compiler_without_sexp.Cmm.rec_flag)
