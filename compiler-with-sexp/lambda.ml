...
type compile_time_constant = Compiler_without_sexp.Lambda.compile_time_constant =
  | Big_endian
  | Word_size
  | Int_size
  | Max_wosize
...
include (
  Compiler_without_sexp.Lambda :
    module type of struct
      include Compiler_without_sexp.Lambda
    end
    with type array_kind := Compiler_without_sexp.Lambda.array_kind
     and type bigarray_kind := Compiler_without_sexp.Lambda.bigarray_kind
     and type bigarray_layout := Compiler_without_sexp.Lambda.bigarray_layout
     and type block_shape := Compiler_without_sexp.Lambda.block_shape
     and type boxed_integer := Compiler_without_sexp.Lambda.boxed_integer
     and type compile_time_constant := Compiler_without_sexp.Lambda.compile_time_constant
     and type float_comparison := Compiler_without_sexp.Lambda.float_comparison
     and type function_attribute := Compiler_without_sexp.Lambda.function_attribute
     and type function_kind := Compiler_without_sexp.Lambda.function_kind
     and type immediate_or_pointer := Compiler_without_sexp.Lambda.immediate_or_pointer
     and type initialization_or_assignment :=
          Compiler_without_sexp.Lambda.initialization_or_assignment
     and type inline_attribute := Compiler_without_sexp.Lambda.inline_attribute
     and type integer_comparison := Compiler_without_sexp.Lambda.integer_comparison
     and type is_safe := Compiler_without_sexp.Lambda.is_safe
     and type lambda := Compiler_without_sexp.Lambda.lambda
     and type lambda_apply := Compiler_without_sexp.Lambda.lambda_apply
     and type lambda_event := Compiler_without_sexp.Lambda.lambda_event
     and type lambda_event_kind := Compiler_without_sexp.Lambda.lambda_event_kind
     and type lambda_switch := Compiler_without_sexp.Lambda.lambda_switch
     and type let_kind := Compiler_without_sexp.Lambda.let_kind
     and type lfunction := Compiler_without_sexp.Lambda.lfunction
     and type local_attribute := Compiler_without_sexp.Lambda.local_attribute
     and type meth_kind := Compiler_without_sexp.Lambda.meth_kind
     and type primitive := Compiler_without_sexp.Lambda.primitive
     and type program := Compiler_without_sexp.Lambda.program
     and type raise_kind := Compiler_without_sexp.Lambda.raise_kind
     and type shared_code := Compiler_without_sexp.Lambda.shared_code
     and type specialise_attribute := Compiler_without_sexp.Lambda.specialise_attribute
     and type structured_constant := Compiler_without_sexp.Lambda.structured_constant
     and type value_kind := Compiler_without_sexp.Lambda.value_kind)
